...
def test_query_plan_dag() -> None:
    ...
    plan = QueryPlan(query_ir_id="qir_123", steps=[step1, step2, step3])

    assert len(plan.steps) == 3
    assert plan.get_step("step_zonal_stats").dependencies == [
        "step_fetch_hospitals",
        "step_fetch_vegetation",
    ]
...
def test_analytics_results_in_response() -> None:
    ...
    table = TabularData(
        ...
        rows=[["Imam Hospital", 0.62, "High"], ["Bakhshande Hospital", 0.21, "Low"]],
    )
    ...
    analytics = AnalyticsResult(
        metrics=[metric], tables=[table], aggregations=[aggregation]
    )

    response = GeoResponse.success(
        features=[], query_ir_id="qir_complex", analytics=analytics
    )
